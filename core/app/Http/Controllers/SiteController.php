...
class SiteController extends Controller {
    ...
    public function checkUser(Request $request) {
        ...
        if ($request->email) {
            $exist['data'] = Customer::where('email', $request->email)->exists();
            $exist['type'] = 'email';
        }
        if ($request->mobile) {
            $exist['data'] = Customer::where('mobile', $request->mobile)->exists();
            $exist['type'] = 'mobile';
        }
        if ($request->username) {
    ...
    public function checkGuarantor(Request $request) {
        $exists = Guarantor::where('mobile', $request->mobile)->orWhere('cellphone', $request->mobile)->exists();
        $data  = $exists;
        if($exists){
            $data = Guarantor::where('mobile', $request->mobile)->orWhere('cellphone', $request->mobile)->first();
        }
        return [
            'type'  =>  'guarantor',
            'data'  =>  $data
