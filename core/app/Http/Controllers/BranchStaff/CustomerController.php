...
class CustomerController extends Controller {
    ...
    public function edit($customerId) {
        ...
        // Check if staff has permission to access this customer's branch
        $canChangeStatus = in_array($customer->branch_id, $staff->assignBranch->pluck('id')->toArray());

        if (!$canChangeStatus) {
            $notify[] = ['error', 'You do not have permission to edit this customer'];
            return back()->withNotify($notify);
        }
        ...
        return view('branch_staff.user.customer_edit', compact(
            'pageTitle',
            'customer',
            'staff',
            'countries',
