...
class BranchStaffController extends Controller {
    ...
    public function managerDashboard() {
        ...
        $transactions      = CustomerTransaction::where('branch_id', $branch->id)
            // ->with('user:id,account_number,firstname,lastname', 'branchStaff:id,name')
            ->with('account', 'branchStaff')
            ->latest()
            ->limit(10)
            ->get();

        $depositedAmount   = CustomerTransaction::where('branch_id', $branch->id)->where('dr_cr', 1)->where('status', 1)->whereDate('created_at', today())->sum('amount');
        $withdrawnAmount   = CustomerTransaction::where('branch_id', $branch->id)->where('dr_cr', 2)->where('status', 1)->whereDate('created_at', today())->sum('amount');

        return view('branch_staff.dashboard.manager', compact('pageTitle', 'account_category', 'staff', 'branch', 'transactions', 'depositedAmount', 'withdrawnAmount'));
    }
    ...
    public function transactions() {
        ...
        if (authStaff()->designation == Status::ROLE_TELLER) {
