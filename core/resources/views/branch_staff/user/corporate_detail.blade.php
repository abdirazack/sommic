...
@section('panel')
    <div class="d-flex gap-3 mb-4 flex-wrap">
        <div class="flex-fill">
            <x-widget style="2" color="primary" title="Account Number | Account Type" value="{{ @$account->account_number }} | Corporate {{ @$account->AccountType->name }}" icon="la la-user" icon_style="solid" />
        </div>
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title text-center">@lang('Account Features')</h5>
                </div>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-xl-7 mt-2">
                            <div class="row">
                                <h6 class="col-xl-6">
                                    Registered by: {{ ucwords(@$account->branchStaff->name) }}
                                </h6>
                                <h6 class="col-xl-6">
                                    Joined on: {{ showDateTime(@$account->created_at, 'd M Y, h:i A') }}
                                </h6>
                            </div>
                        </div>
                        <div class="col-xl-1">
                            <div class="row">
                                <div class="col-12">
                                    @if ($account->status == 0)
                                        <span class="bg--warning py-1 px-2 rounded float-end"> <i class="la la-check-circle"></i> @lang('Pending')</span>
                                    @elseif($account->status == 1)
                                        <span class="bg--success py-1 px-2 rounded float-end"> <i class="la la-check-circle"></i> @lang('Active')</span>
                                    @else
                                        <span class="bg--danger py-1 px-2 rounded float-end"> <i class="la la-ban"></i> @lang('Banned')</span>
                                    @endif
                                </div>
                            </div>
                        </div>
    ...
    <div class="row gy-4 mb-4">
                ...
                <div class="card-body mb-2">
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item d-flex justify-content-between flex-wrap border-0">
                            ...
                            <a class="viewable" href="#" src='{{ getImage(getFilePath("userProfile") . "/" . @$account->misc->documents->{'p' . ($loop->index + 1) . '_signature_scan'}, null, true) }}'>@lang('View')</a>
                        </div>
        ...
        <div class="col-xl-6 col-md-6">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title text-center">@lang('Person ' . ($loop->index + 1))</h5>
                </div>
                <div class="card-body">
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item border-0">
                            ...
                            <h6><a href="{{ route('staff.customer.detail', @$customer->id) }}">{{ @$customer->name }}</a></h6>
                        </div>
                        ...
                        <div class="list-group-item border-0">
                            <div class="row">
                                <div class="col-5">
                                    ...
                                    <h6>{{ @$customer->misc->gender == 1? "Male" : "Female" }}</h6>
                                </div>
                                <div class="col-7">
                        ...
                        <div class="list-group-item border-0">
                            ...
                            <h6>{{ strtoupper(@$countries[@$customer->misc->nationality]->country) }}</h6>
                        </div>
        ...
        <div class="col-xl-3 col-md-3">
            @foreach($account->misc->nok->{'p' . ($loop->index + 1) . '_nok'} as $nok)
            <div class="card mb-4">
                <div class="card-header">
                    <h5 class="card-title text-center">@lang('Next of Kin ' . ($loop->index + 1))</h5>
                </div>
                <div class="card-body">
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item border-0">
                            <small class="text-muted">@lang('Name')</small>
                            <h6>{{ ucwords(@$account->misc->nok->{'p' . ($loop->parent->index + 1) . '_nok'}->{'nok' . ($loop->index + 1)}->name) }}</h6>
                        </div>
                        
                        <div class="list-group-item border-0">
                            <small class="text-muted">@lang('Relation')</small>
                            <h6>{{ @$relations[@$account->misc->nok->{'p' . ($loop->parent->index + 1) . '_nok'}->{'nok' . ($loop->index + 1)}->relation] }} </h6>
                        </div>

                        <div class="list-group-item  border-0">
                            <small class="text-muted">@lang('Mobile Number')</small>
                            <h6>{{ @$account->misc->nok->{'p' . ($loop->parent->index + 1) . '_nok'}->{'nok' . ($loop->index + 1)}->mobile }} </h6>
                        </div>
